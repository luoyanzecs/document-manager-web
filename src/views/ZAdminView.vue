      ...
      <div class="h-screen overflow-auto">
        <div v-if="tableProp.pairs.length === 0" class="pos-center mt-60">
          <svg class="text-gray-400 w-20 h-20 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647"></path>
          </svg>
        </div>
        <template v-else>
  ...
  <transition-group name="fade">
    ...
    <z-dailog :key="3" v-if="BUTTON_LOADER.isAddUserDailogShow">
      ...
      <template #body>
        ...
        <form class="mb-4">
          <table class="table-fixed text-gray-500 border-separate" style="border-spacing: 0.5rem 1rem">
            <tr>
              ...
              <td>
                <input type="text" class="new-user-input" spellcheck="false" @input="$event.target.required = 'required'"
                       pattern="[a-z|A-Z|.|0-9|@]{6,16}" placeholder="请输入用户名" v-model="NEW_USER_PARAMS.name"/>
              </td>
            </tr>
            <tr>
              ...
              <td>
                <input type="text" class="new-user-input" spellcheck="false" @input="$event.target.required = 'required'"
                       pattern="[a-z|A-Z|.|0-9|@]{6,16}" placeholder="请输入新用户密码" v-model="NEW_USER_PARAMS.password"/>
              </td>
            </tr>
            <tr>
              <td class="border-r-2 pr-2">部门</td>
              <td>
                <span v-for="bu in buList" :key="bu" @click="NEW_USER_PARAMS.bu = bu.id"
                      :class="[{ 'selected-item': NEW_USER_PARAMS.bu === bu.id }, 'select-item']">
                  {{ bu.name }}
                </span>
              </td>
            </tr>
            <tr>
              ...
              <td class="space-x-2">
                <span :class="[{ 'selected-item': !NEW_USER_PARAMS.isAdmin },'select-item',]" @click="NEW_USER_PARAMS.isAdmin = false"
                >用户</span>
                <span :class="[{ 'selected-item': NEW_USER_PARAMS.isAdmin }, 'select-item',]" @click="NEW_USER_PARAMS.isAdmin = true"
                >管理员</span>
              </td>
            </tr>
            <tr>
              ...
              <td>
                <input type="number" class="new-user-input" min="0" max="10" v-model="NEW_USER_PARAMS.rank" :disabled="NEW_USER_PARAMS.isAdmin"/>
              </td>
            </tr>
          </table>
        </form>
    ...
    <z-dailog :key="4" v-if="BUTTON_LOADER.isAddNoticeDailogShow">
      ...
      <template #body>
        <ul class="space-y-1">
          ...
          <li><p class="text-gray-400 text-xs">* 日期范围为今天 至 2030-12-31。</p>
          </li>
        </ul>
        <form class="mb-4 select-none">
          <table class="table-fixed text-gray-500 border-separate" style="border-spacing: 1rem 1rem">
            <tr>
              ...
              <td>
                <span :class="[{ 'selected-item': NEW_NOTICE_PARAMS.isGlobal }, 'select-item']"
            ...
            <tr>
              ...
              <td class="font-mono">
                <input class="focus:outline-none border rounded-lg px-2" max="2030-12-31" :min="transformTimeShort()" type="date" v-model="NEW_NOTICE_PARAMS.startTime" pattern="yyyy-mm-dd" @input="$event.target.required = 'required'"
                />
              </td>
            </tr>
            <tr>
              ...
              <td class="font-mono select-none">
                <input class="focus:outline-none border rounded-lg px-2" max="2030-12-31" :min="transformTimeShort()" type="date" v-model="NEW_NOTICE_PARAMS.expiredTime" @input="$event.target.required = 'required'"
                />
              </td>
            </tr>
            <tr>
              ...
              <td class="w-full">
                <textarea class=" focus:outline-none border rounded-lg px-2 py-2 resize-x" spellcheck="false" @input="$event.target.required = 'required'"
                          rows="2" minlength="5" maxlength="60" placeholder="请输入通知内容" v-model="NEW_NOTICE_PARAMS.text"/>
              </td>
            </tr>
          </table>
        </form>
      ...
      <template #bottom>
        <z-button fill="确认" @click="addNoticeHandler" :load-visible="BUTTON_LOADER.isAddNoticeConfirmBtn" :deny-animation="BUTTON_LOADER.isAddNoticeConfirmBtnShake"/>
...
const SEARCH_PARAMS = reactive({
  ...
  page: 1,
  pageSize: 20,
  bu: computed(() => buList.value.filter(it => it && it.checked).map(it => it.id)),
  userids: [],
  clearAll: function () {
    this.page = 1;
    this.userids = [];
...
const pageProp = reactive({ currentPage: 1, visible: false, totalPage: 0 });

const tableProp = reactive({ pairs: [], items: [] });
...
const setProps = (res, selectPage) => {
  ...
  pageProp.totalPage = res.totalPage || 0
...
function deleteItemHandler() {
  ...
  const param = { menuIndex: SEARCH_PARAMS.menuIndex,
    ids: tableProp.items
        .filter((it, idx) => table.value.isSelect[idx])
        .map((it) => it.id),
  }
  DELETE_TABLE_ITEMS(param)
      .then(() => selectPageHandler(SEARCH_PARAMS.page, false))
      .finally(() => {
...
function selectHandler() {
  return ADMIN_SEARCH_ALL_IN_ONE(SEARCH_PARAMS).then((it) =>
...
.select-item {
  @apply px-3 py-0.5 border rounded-2xl cursor-pointer whitespace-nowrap mx-2;
}
