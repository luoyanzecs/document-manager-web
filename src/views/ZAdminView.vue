      ...
      <template v-slot:context>
        ...
        >
          <div
            ...
            :class="{
              'select-menu': index === SEARCH_PARAMS.menuIndex,
              'menu-base': true,
            }"
          >
            {{ item }}
    ...
    <div class="flex-auto relative h-screen min-w-0">
        ...
        <template v-slot:tools>
          <z-head-menu>
            <template v-if="SEARCH_PARAMS.menuIndex === 0">
              <z-button
                fill="新增"
                @click="headButtonHandler('10')"
                :load-visible="BUTTON_LOADER._10"
              />
              <z-button
                fill="部门"
                @click="headButtonHandler('00')"
                :load-visible="BUTTON_LOADER._00"
              />
              <z-button
                fill="删除"
                @click="headButtonHandler('01')"
                :load-visible="BUTTON_LOADER._01"
                :deny-animation="BUTTON_LOADER.isDeleteBtnShake"
              />
              <z-button
                fill="所有"
                @click="headButtonHandler('22')"
                :load-visible="BUTTON_LOADER._22"
              />
            </template>
            ...
            <template v-if="SEARCH_PARAMS.menuIndex === 3">
              ...
              <z-button
                fill="新增"
                @click="headButtonHandler('30')"
                :load-visible="BUTTON_LOADER._30"
              />
              <z-button
                fill="删除"
                @click="headButtonHandler('01')"
      ...
      <div class="h-screen overflow-auto">
        <div v-if="tableProp.pairs.length === 0" class="pos-center mt-60">
          <svg
            class="text-gray-400 w-20 h-20 animate-spin"
  ...
  <transition-group name="fade">
    <z-dailog :key="1" v-show="BUTTON_LOADER.isDeleteDailogShow">
      ...
      <template #bottom>
        <z-button
          ...
          :load-visible="BUTTON_LOADER.deleteConfirmBtn"
        />
        <z-button
          fill="取消"
          @click="BUTTON_LOADER.isDeleteDailogShow = false"
        />
      </template>
    </z-dailog>
    <z-dailog
      :key="2"
      v-show="BUTTON_LOADER.isBuSelectDailogShow"
      v-model:click-toggle="BUTTON_LOADER.isBuSelectDailogShow"
    >
      ...
      <template #body>
        <div class="flex gap-2 mb-6">
          <div
            v-for="bu in buList"
            :key="bu.name"
            @click="bu.checked = !bu.checked"
            :class="[
              { 'bg-yellow-500 bg-opacity-30': bu.checked },
              'px-3',
              'py-0.5',
              'border',
              'rounded-2xl',
      ...
      <template #bottom>
        <z-button
          fill="确认"
          @click="buSelectHandler"
          :load-visible="BUTTON_LOADER.buSelectConfirmBtn"
        />
        <z-button
          fill="取消"
          @click="BUTTON_LOADER.isBuSelectDailogShow = false"
        />
      </template>
    </z-dailog>
    <z-dailog :key="3" v-if="BUTTON_LOADER.isAddUserDailogShow">
      ...
      <template #body>
        ...
        <form class="mb-4">
          <table
            class="table-fixed text-gray-500 border-separate"
            style="border-spacing: 0.5rem 1rem"
          >
            <tr>
              ...
              <td>
                <input
                  ...
                  spellcheck="false"
                  @input="$event.target.required = 'required'"
                  pattern="[a-z|A-Z|.|0-9|@]{6,16}"
                  placeholder="请输入用户名"
                  v-model="NEW_USER_PARAMS.name"
                />
              </td>
            </tr>
            <tr>
              ...
              <td>
                <input
                  ...
                  v-model="NEW_USER_PARAMS.password"
                />
              </td>
            </tr>
            <tr>
              ...
              <td>
                <span
                  v-for="bu in buList"
                  :key="bu"
                  @click="NEW_USER_PARAMS.bu = bu.name"
                  :class="[
                    { 'selected-item': NEW_USER_PARAMS.bu === bu.name },
                    'select-item',
                  ]"
                  >{{ bu.name }}</span
                >
            ...
            <tr>
              ...
              <td class="space-x-2">
                <span
                  :class="[
                    { 'selected-item': !NEW_USER_PARAMS.isAdmin },
                    'select-item',
                  ]"
                  @click="NEW_USER_PARAMS.isAdmin = false"
                  >用户</span
                >
                <span
                  :class="[
                  ...
                  @click="NEW_USER_PARAMS.isAdmin = true"
                  >管理员</span
                >
              </td>
            </tr>
            <tr>
              ...
              <td>
                <input
                  ...
                  min="0"
                  max="10"
                  v-model="NEW_USER_PARAMS.rank"
                  :disabled="NEW_USER_PARAMS.isAdmin"
                />
              </td>
            </tr>
          </table>
      ...
      <template #bottom>
        <z-button
          ...
          :load-visible="BUTTON_LOADER.isAddUserConfirmBtn"
          :deny-animation="BUTTON_LOADER.isAddUserConfirmBtnShake"
        />
        <z-button
          fill="取消"
          @click="BUTTON_LOADER.isAddUserDailogShow = false"
        />
      </template>
    </z-dailog>
    <z-dailog :key="4" v-if="BUTTON_LOADER.isAddNoticeDailogShow">
      ...
      <template #body>
        ...
        <form class="mb-4 select-none">
          <table
            class="table-fixed text-gray-500 border-separate"
            style="border-spacing: 1rem 1rem"
          >
            <tr>
              ...
              <td>
                <span
                  :class="[
                    { 'selected-item': NEW_NOTICE_PARAMS.isGlobal },
                    'select-item',
                  ]"
                  @click="
                    NEW_NOTICE_PARAMS.isGlobal = !NEW_NOTICE_PARAMS.isGlobal
                  "
                  >是</span
                >
              </td>
            ...
            <tr>
              ...
              <td class="space-x-2">
                <span
                  ...
                  :key="bu"
                  @click="
                    NEW_NOTICE_PARAMS.bu.includes(bu.name)
                      ? NEW_NOTICE_PARAMS.bu.splice(
                          NEW_NOTICE_PARAMS.bu.indexOf(bu.name),
                          1
                        )
                      : NEW_NOTICE_PARAMS.bu.unshift(bu.name)
                  "
                  :class="[
                    { 'selected-item': NEW_NOTICE_PARAMS.bu.includes(bu.name) },
                    'select-item',
                  ]"
                  >{{ bu.name }}</span
                >
            ...
            <tr>
              ...
              <td class="font-mono">
                <input
                  class="focus:outline-none border rounded-lg px-2"
                  max="2030-12-31"
                  :min="transformTimeShort()"
                  type="date"
                  v-model="NEW_NOTICE_PARAMS.startTime"
                  pattern="yyyy-mm-dd"
                  @input="$event.target.required = 'required'"
                />
              </td>
            ...
            <tr>
              ...
              <td class="font-mono select-none">
                <input
                  ...
                  :min="transformTimeShort()"
                  type="date"
                  v-model="NEW_NOTICE_PARAMS.expiredTime"
                  @input="$event.target.required = 'required'"
                />
              </td>
            </tr>
      ...
      <template #bottom>
        <z-button
          fill="确认"
          @click="addNoticeHandler"
          :load-visible="BUTTON_LOADER.isAddNoticeConfirmBtn"
          :deny-animation="BUTTON_LOADER.isAddNoticeConfirmBtnShake"
        />
        <z-button
          fill="取消"
          @click="BUTTON_LOADER.isAddNoticeDailogShow = false"
        />
      </template>
    </z-dailog>
  </transition-group>
...
const NEW_USER_PARAMS = reactive({
  ...
  get: function () {
    // let x
    // console.log(typeof(NEW_USER_PARAMS.bu))
    // switch (NEW_USER_PARAMS.bu) {
    //   case "开发":
    //     x = 1;
    //     break;
    //   case "运营":
    ...
    return {
      ...
      bu: (buList.value.find(it => it.name === NEW_USER_PARAMS.bu).id),
...
const BUTTON_LOADER = reactive({
  ...
  _22: false,
  _30: false,
  isDeleteBtnShake: false,
  isDeleteDailogShow: false,
  deleteConfirmBtn: false,
  isBuSelectDailogShow: false,
  buSelectConfirmBtn: false,
  isAddUserDailogShow: false,
  isAddUserConfirmBtn: false,
  isAddUserConfirmBtnShake: false,
  isAddNoticeDailogShow: false,
  isAddNoticeConfirmBtn: false,
  isAddNoticeConfirmBtnShake: false,
});
...
function addUserHandler() {
  ...
  if (
    !regExp.test(NEW_USER_PARAMS.name) ||
    !regExp.test(NEW_USER_PARAMS.password) ||
    NEW_USER_PARAMS.bu === ""
  ) {
    sleep(800, () => {
  ...
  ADD_NEW_USER(NEW_USER_PARAMS.get())
    .then(() =>
      store.commit("unshiftNotice", { type: 1, message: "新用户已添加" })
    )
    .finally(() => {
...
function addNoticeHandler() {
  if (
    NEW_NOTICE_PARAMS.text.length < 5 ||
    NEW_NOTICE_PARAMS.text.length > 60 ||
    NEW_NOTICE_PARAMS.expiredTime === ""
  ) {
    sleep(800, () => {
  ...
  ADD_NEW_NOTICE(NEW_NOTICE_PARAMS.get())
    .then(() =>
      store.commit("unshiftNotice", { type: 1, message: "新通知已添加" })
    )
    .finally(() => {
...
function deleteItemHandler() {
  const params = {
    menuIndex: SEARCH_PARAMS.menuIndex,
    ids: tableProp.items
      .filter((it, idx) => table.value.isSelect[idx])
      .map((it) => it.id),
  };
  BUTTON_LOADER.deleteConfirmBtn = true;
  DELETE_TABLE_ITEMS(params)
    .then(() => {
...
function selectHandler() {
  return ADMIN_SEARCH_ALL_IN_ONE(SEARCH_PARAMS.get()).then((it) =>
    setProps(it, SEARCH_PARAMS.page)
  );
}
  ...
  switch (index) {
    ...
    case "01": {
      ...
      if (table.value.isSelect.filter((it) => it).length === 0) {
        sleep(800, () => (BUTTON_LOADER.isDeleteBtnShake = true)).then(
          () => (BUTTON_LOADER.isDeleteBtnShake = false)
        );
        break;
      }
...
.fade-leave-active {
  animation: fade ease-in-out 0.15s reverse;
}
